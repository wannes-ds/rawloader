        ...
        for i in 0..dircount {
            ...
            if e.typ == 0x2800 || e.typ == 0x3000 {
                ...
                if depth < 10 {
                    ...
                    let ifd =
                        CiffIFD::new(buf, e.data_offset, e.data_offset + e.bytesize, depth + 1);
                    match ifd {
                        Ok(val) => {
                            subifds.push(val);
                        }
    ...
    pub fn new(buf: &'a [u8], value_data: usize, offset: usize) -> Result<CiffEntry<'a>, String> {
        ...
        let (bytesize, data_offset) = match datalocation {
            ...
            0x0000 => (
                ...
                LEu32(buf, offset + 6) as usize + value_data,
            ),
            // Data is stored directly in entry
            0x4000 => (8, offset + 2),
            val => {
                return Err(format!("CIFF: Don't know about data location {:x}", val).to_string())
            }
        };
