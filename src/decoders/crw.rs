...
impl<'a> Decoder for CrwDecoder<'a> {
    fn image(&self, dummy: bool) -> Result<RawImage, String> {
        ...
        let camera =
            self.rawloader
                .check_supported_with_everything(&makemodel[0], &makemodel[1], "")?;

        let (width, height, image) = if camera.model == "Canon PowerShot Pro70" {
            (
                1552,
                1024,
                decode_10le_lsb16(&self.buffer[26..], 1552, 1024, dummy),
            )
        } else {
            ...
            (
                width,
                height,
                self.decode_compressed(&camera, width, height, dummy)?,
            )
        };
...
impl<'a> CrwDecoder<'a> {
    fn get_wb(&self, cam: &Camera) -> Result<[f32; 4], String> {
        ...
        if !cam.find_hint("nocinfo2") {
            if let Some(cinfo) = self.ciff.find_entry(CiffTag::ColorInfo2) {
                return Ok(if cinfo.get_u32(0) > 512 {
                    ...
                    [
                        cinfo.get_f32(51),
                        (cinfo.get_f32(50) + cinfo.get_f32(53)) / 2.0,
                        cinfo.get_f32(52),
                        NAN,
                    ]
                });
        ...
        if let Some(cinfo) = self.ciff.find_entry(CiffTag::ColorInfo1) {
            ...
            let key: [u16; 2] = if cam.find_hint("wb_mangle") {
                [0x410, 0x45f3]
            } else {
                [0, 0]
            };
