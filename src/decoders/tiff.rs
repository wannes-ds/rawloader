...
impl<'a> TiffIFD<'a> {
                ...
                || entry.tag == t(Tag::KdcIFD)
            {
                if depth < 10 {
                    ...
                    for i in 0..entry.count {
                        let ifd = TiffIFD::new(
                            buf,
                            entry.get_u32(i as usize) as usize,
                            base_offset,
                            start_offset,
                            depth + 1,
                            e,
                ...
                if depth < 10 {
                    // Avoid infinite looping IFDs
                    let ifd =
                        TiffIFD::new_makernote(buf, entry.doffset(), base_offset, depth + 1, e);
                    match ifd {
                        Ok(val) => {
                            subifds.push(val);
                        }
    ...
    pub fn new_makernote(
        ...
        if data[0..6] == b"PENTAX"[..] {
            off += 8;
            let endian = if data[off..off + 2] == b"II"[..] {
                LITTLE_ENDIAN
            } else {
                BIG_ENDIAN
            };
    ...
    pub fn entries(&self) -> &HashMap<u16, TiffEntry> {
        &self.entries
    }
